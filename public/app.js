...
async function renderFacturacion() {
  // Asegurar que turnoActivoCajero esté actualizado ANTES de facturar.
  // Sin esto, las ventas se guardan con turno_id=null y el cierre de turno
  // reporta 0 ventas aunque sí se haya facturado.
  try {
    const ta = await GET('/turnos/activo');
    turnoActivoCajero = ta || null;
  } catch(e) { turnoActivoCajero = null; }

  // Mostrar aviso si el cajero no tiene turno abierto (no aplica a admin/supervisor sin turno asignado)
  const banner = document.getElementById('fac-sin-turno-banner');
  if (banner) {
    const requiereTurno = USER?.rol === 'cajero';
    banner.style.display = (requiereTurno && !turnoActivoCajero) ? 'flex' : 'none';
  }
...
async function processFacInvoice() {
  ...
  try {
    ...
    // Re-verificar el turno activo justo antes de facturar — evita que una
    // factura quede sin turno_id si pasó tiempo desde que se cargó la pantalla.
    try {
      const ta = await GET('/turnos/activo');
      turnoActivoCajero = ta || null;
    } catch(e) { /* si falla, se usa el valor que ya estaba en memoria */ }
...
function descargarPlantilla(tipo) {
  ...
  const instrSheet = [
    ...
    ['5. En Petra POS → módulo correspondiente → botón "Importar Excel"'],
    ...cfg.notas
  ];
...
async function imprimirCorte(turnoId) {
  try {
    ...
    <body>
      ...
      `:''}
      <div class="center" style="margin-top:8px;font-size:10px;color:#666">Powered by PetraPOS</div>
...
async function cerrarTurnoCajero(id, efectivo_contado, sobrante=0, motivo_sobrante='') {
  try {
    ...
    try {
      ...
      // Crear ticket automáticamente (en paralelo, sin bloquear)
...
function _textoCorte(turno, ventas) {
  ...
  const lines = [
    ...
    sep32,
    'Powered by PetraPOS'.padStart(26),
    sep32,
  ];
  return '```\n' + lines.join('\n') + '\n```';
}
...
async function _textoArticulos(turnoId, turno, ventas) {
  ...
  lines.push(sep);
  lines.push('Powered by PetraPOS'.padStart(W-2));
  lines.push(sep);
...
function _htmlCortePDF(turno, ventas) {
  ...
  return `
    ...
    <p style="text-align:center;margin-top:16px;font-size:10px;color:#94a3b8">Powered by PetraPOS</p>`;
}
...
async function probarCallmebot(id, numero, apikey) {
  const empresa = USER?.empresa || 'PetraPOS';
...
function imprimirConsolidado(fecha) {
  ...
  html += `<div class="sep2"></div>`;
  html += `<div class="center" style="font-size:10px;margin-top:6px">Powered by PetraPOS</div>`;
...
async function enviarConsolidadoWA(fecha) {
  ...
  lines1.push('_Powered by PetraPOS_');
  const textoCorte = '```\n' + lines1.join('\n') + '\n```';
  ...
  lines2.push('_Powered by PetraPOS_');
  const textoArts = '```\n' + lines2.join('\n') + '\n```';

  // Generar botones para cada número (con TextMeBot si tiene API Key)
  const _rawCorte = textoCorte.replace(/```\n/g,'').replace(/\n```/g,'');
  const _rawArts  = textoArts.replace(/```\n/g,'').replace(/\n```/g,'');
  const botonesHTML = numeros.map(wa => {
    ...
    const tieneKey = !!wa.callmebot_apikey;
    const btnAuto = tieneKey
      ? '<button id="ccbtn-'+tel+'" onclick="_enviarConsolidadoCB(\''+wa.numero+'\',\''+wa.callmebot_apikey+'\',\''+tel+'\')" '
        +'style="width:100%;background:linear-gradient(135deg,#059669,#10b981);color:#fff;border:none;border-radius:8px;'
        +'padding:10px;font-weight:700;font-size:13px;cursor:pointer;margin-bottom:6px">⚡ Enviar automáticamente</button>'
      : '';
    const badge = tieneKey
      ? '<span style="margin-left:auto;background:#dcfce7;color:#059669;font-size:10px;font-weight:700;padding:2px 7px;border-radius:99px">AUTO</span>'
      : '';
  ...
  // Helper: envío automático del consolidado vía TextMeBot
  window._enviarConsolidadoCB = async function(numero, apikey, tid) {
    const btn = document.getElementById('ccbtn-'+tid);
    if (btn) { btn.disabled = true; btn.textContent = '📤 Enviando corte...'; }
    try {
      await POST('/whatsapp/send', { numero, apikey, mensaje: _rawCorte });
      if (btn) btn.textContent = '📤 Enviando artículos...';
      await new Promise(r => setTimeout(r, 5000));
      await POST('/whatsapp/send', { numero, apikey, mensaje: _rawArts });
      if (btn) { btn.textContent = '✅ Enviado'; btn.style.background = '#059669'; }
      showToastMsg('✅ Consolidado enviado correctamente por WhatsApp');
    } catch(e) {
      if (btn) { btn.disabled = false; btn.textContent = '⚡ Enviar automáticamente'; }
      alert('Error TextMeBot: ' + e.message);
    }
  };
    ...
    <div style="background:#fff;width:100%;max-width:520px;display:flex;flex-direction:column;
      ...
      ${(()=>{
        ...
        <div style="border-bottom:1px solid #e2e8f0;padding:12px 18px;flex-shrink:0">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            ${fotos.map((src,i)=>{
              const esPdf = /^data:application\/pdf/.test(src) || /\.pdf($|\?)/i.test(src);
              if (esPdf) {
                return `<a href="${src}" target="_blank" title="Abrir PDF"
                  style="width:60px;height:60px;border-radius:8px;border:1px solid #e2e8f0;
                         background:#fef2f2;display:flex;align-items:center;justify-content:center;
                         font-size:24px;text-decoration:none;flex-shrink:0">📄</a>`;
              }
              return `<img src="${src}" onclick="tkVerFoto('${i}',${JSON.stringify(fotos).replace(/"/g,"'")})"
                style="width:60px;height:60px;object-fit:cover;border-radius:8px;cursor:pointer;
                       border:1px solid #e2e8f0" title="Click para ampliar"
                onerror="this.outerHTML='<div style=\\'width:60px;height:60px;border-radius:8px;border:1px solid #e2e8f0;background:#f1f5f9;display:flex;align-items:center;justify-content:center;font-size:20px\\'>⚠️</div>'">`;
            }).join('')}
          </div>
        </div>` : '';
      })()}
