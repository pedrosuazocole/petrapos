...
// Reparación: reasigna ventas sin turno_id al turno correcto del mismo cajero
// según el rango horario apertura→cierre. Solo Admin puede ejecutarlo.
app.post('/api/turnos/reparar-ventas-huerfanas', auth(['admin']), (req, res) => {
  try {
    const huerfanas = all(`SELECT id,usuario_id,fecha FROM ventas WHERE turno_id IS NULL AND estado='emitida'`);
    let reparadas = 0;
    const detalle = [];
    for (const v of huerfanas) {
      // Buscar el turno del mismo cajero cuyo rango de fechas contiene la venta
      const turno = get(
        `SELECT id, turno_letra FROM turnos
         WHERE usuario_id=?
           AND fecha_apertura<=?
           AND (fecha_cierre IS NULL OR fecha_cierre>=?)
         ORDER BY fecha_apertura DESC LIMIT 1`,
        [v.usuario_id, v.fecha, v.fecha]
      );
      if (turno) {
        run(`UPDATE ventas SET turno_id=? WHERE id=?`, [turno.id, v.id]);
        reparadas++;
        detalle.push({ venta_id: v.id, turno_asignado: turno.turno_letra, fecha: v.fecha });
      }
    }
    if (reparadas > 0) saveDB();
    res.json({ ok: true, total_huerfanas: huerfanas.length, reparadas, sin_match: huerfanas.length - reparadas, detalle });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
// Envío masivo al cerrar turno vía TextMeBot
app.post('/api/whatsapp/send-turno', auth(), async (req, res) => {
  ...
  for (const wa of numeros) {
    try {
      ...
      await new Promise(r => setTimeout(r, 5000));
      resultados.push({ nombre: wa.nombre, ok: true });
    } catch(e) {
      resultados.push({ nombre: wa.nombre, ok: false, error: e.message });
    }
  }
  res.json({ ok: true, enviados: resultados.filter(r=>r.ok).length, total: numeros.length, detalle: resultados });
});
      ...
      if (ticket.turno_id) {
        ...
        try {
          ...
          pdfBase64 = await new Promise((resolve, reject) => {
            ...
            const empresa = ticket.cajero_nombre ? ticket.cajero_nombre : 'PetraPOS';
        ...
        // Token aparte para el adjunto que el usuario subió (foto o PDF de evidencia)
        const adjuntoToken = adjunto_resolucion ? uuid() : null;
        ...
        run(`UPDATE tickets SET
               ...
               fotos=?
             WHERE id=?`,
          [estado, resolucion||null, pdfBase64, pdfToken, adjuntoToken, JSON.stringify(fotasFinales), req.params.id]
        );
        saveDB();
        ...
        if (numeros.length) {
          ...
          // Link público del PDF sin autenticación, válido 24h
          const pdfPublicUrl = pdfToken ? `${serverUrl}/public/pdf/${pdfToken}` : null;
          // Link público del adjunto que el usuario subió (foto o documento de evidencia)
          const adjuntoPublicUrl = adjuntoToken ? `${serverUrl}/public/adjunto/${adjuntoToken}` : null;

          const msg = `✅ *PetraPOS — Ticket Resuelto*\n`
          ...
          (async () => {
            for (const wa of numeros) {
              try {
                ...
                await new Promise(r => setTimeout(r, 5000));
                // Si hay PDF, enviarlo adjunto directamente
                if (pdfPublicUrl) {
                  const pdfNombre = `Resolucion_Turno_${ticket.turno_letra||'A'}.pdf`;
                ...
                // Si el supervisor adjuntó evidencia (foto/PDF), enviarla también
                if (adjuntoPublicUrl) {
                  const extAdj = (adjunto_resolucion||'').includes('application/pdf') ? 'pdf' : 'jpg';
                  const adjNombre = `Evidencia_Turno_${ticket.turno_letra||'A'}.${extAdj}`;
                  await _textmebotEnviar({
                    numero: wa.numero,
                    apikey: wa.callmebot_apikey,
                    pdfUrl: adjuntoPublicUrl,
                    pdfNombre: adjNombre,
                    mensaje: `📎 Evidencia adjunta del turno ${ticket.turno_letra||'A'}`
                  });
                  await new Promise(r => setTimeout(r, 5000));
                }
        ...
        return res.json({ ok:1, pdf_generado: !!pdfBase64 });
      }
    }
...
// Descarga pública del adjunto que el supervisor subió al resolver (foto o PDF de evidencia).
// El adjunto se guarda como data-URL (data:<mime>;base64,<datos>) dentro de la columna `fotos`,
// así que aquí extraemos el mime real en vez de asumir que siempre es imagen.
app.get('/public/adjunto/:token', (req, res) => {
  try {
    const ticket = get(
      `SELECT fotos, adjunto_token, turno_letra, estado,
              datetime(creado, '+24 hours') as expira
       FROM tickets WHERE adjunto_token=? AND estado='resuelto'`,
      [req.params.token]
    );
    if (!ticket?.fotos) {
      return res.status(404).send('<h2>Adjunto no disponible o link expirado</h2>');
    }
    const ahora = new Date(new Date().getTime() - 6*3600000).toISOString().replace('T',' ').substring(0,19);
    if (ticket.expira && ahora > ticket.expira) {
      return res.status(410).send('<h2>Este link ha expirado (válido 24 horas)</h2>');
    ...
    const dataUrl = fotos[fotos.length - 1]; // el adjunto de la resolución se agrega al final
    if (!dataUrl) return res.status(404).send('<h2>Adjunto no encontrado</h2>');

    const match = /^data:([^;]+);base64,(.+)$/.exec(dataUrl);
    if (!match) return res.status(415).send('<h2>Formato de adjunto no reconocido</h2>');
    const [, mime, base64Data] = match;
    const buf = Buffer.from(base64Data, 'base64');
    const ext = mime === 'application/pdf' ? 'pdf' : (mime.split('/')[1] || 'jpg');
    const nombre = `Evidencia_Turno_${ticket.turno_letra||'A'}.${ext}`;
